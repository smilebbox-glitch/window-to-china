...
- мобильная версия имеет отдельную горизонтальную навигацию ко всем основным разделам;
...
Откройте HTTPS-адрес в Chrome. При доступной установке интерфейс покажет кнопку установки либо браузер предложит её самостоятельно.

### iPhone / iPad

На iOS стандартное событие `beforeinstallprompt` обычно недоступно, поэтому приложение само показывает компактную подсказку: **«Поделиться» → «На экран Домой»**. Подсказку можно закрыть; в установленном standalone-режиме она больше не появляется.
