...
test("Node runtime provides SQLite required by the pilot",()=>{const db=new DatabaseSync(":memory:");db.exec("CREATE TABLE t(v INTEGER); INSERT INTO t VALUES(1)");assert.equal(db.prepare("SELECT v FROM t").get().v,1);db.close();});
test("pilot packages persistent cache and scheduler",async()=>{const pkg=JSON.parse(await text("package.json"));assert.match(pkg.version,/^\d+\.\d+\.\d+-pilot$/u);assert.match(await text("lib/pilot-db.ts"),/source_snapshots/u);assert.match(await text("lib/source-cache.ts"),/stale_until/u);assert.match(await text("compose.yaml"),/china-auto-radar-scheduler/u);assert.match(await text("app/api/internal/refresh/route.ts"),/x-scheduler-token|isSchedulerRequest/u);});
