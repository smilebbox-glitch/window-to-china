...
const pkg=JSON.parse(await readFile("package.json","utf8"));const db=await readFile("lib/pilot-db.ts","utf8");const userStore=await readFile("lib/user-store.ts","utf8");const context=await readFile("lib/user-context.ts","utf8");const refresh=await readFile("app/api/internal/refresh/route.ts","utf8");const shell=await readFile("components/site-shell.tsx","utf8");const tripNav=await readFile("components/trip-workspace-nav.tsx","utf8");const myPage=await readFile("app/my/page.tsx","utf8");const env=await readFile(".env.example","utf8");
...
check(await exists("app/api/user/offline-pack/route.ts"),"offline travel pack backend packaged");
check(await exists("components/favorite-button.tsx")&&await exists("components/user-hub.tsx"),"favorite and personal backend-compatible UX components packaged");
...
check(!shell.includes('href: "/my"')&&!tripNav.includes('href: "/my"')&&myPage.includes('redirect("/")'),"retired personal workspace is not exposed in navigation and legacy route redirects safely");
check(!shell.match(/15\s*мин/iu),"retired 15-minute header badge is not exposed");
