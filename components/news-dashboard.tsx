...
const patterns: Array<{ topic: Topic; pattern: RegExp; weight: number }> = [
  {
    topic: "Геополитика",
    pattern:
      /санкц|пошлин|тариф|экспортн|импортн|огранич|запрет|правитель|минпром|регулир|закон|комплаенс|международ|геополит|关税|制裁|出口管制|政策/iu,
    weight: 30,
  },
  {
    topic: "Стратегия",
    pattern: /стратег|инвестиц|руковод|президент|партнер|альянс|сделк|развити|план|реорганиз|strategy|investment|合作|战略/iu,
    weight: 24,
  },
  ...
  {
    topic: "Технологии",
    pattern: /технолог|батаре|электро|гибрид|двигател|трансмисс|автопилот|безопасност|technology|电池|智能驾驶|新能源/iu,
    weight: 10,
  },
];
...
function isDisplayableNews(item: NewsWithReceipt) {
  ...
  if (
    /не удалось (?:получить|загрузить|извлечь|прочитать).{0,40}(?:текст|новост|материал)|текст (?:недоступен|не получен)|content unavailable|failed to (?:load|fetch|extract|read)|unable to (?:load|fetch|extract|read)/iu.test(
      summary,
    )
  )
    return false;
...
function mergeNews(live: NewsWithReceipt[], seeds: NewsItem[]) {
  const merged = new Map<string, NewsWithReceipt>();
  for (const item of [...live, ...seeds]) {
...
function rateNews(item: NewsWithReceipt): RatedNews {
  ...
  const score = Math.min(
    100,
    Math.round(focusScore(item) + recencyScore + detectedTopics.reduce((total, topic) => total + topic.weight, 0)),
  );
...
/**
 * `seeds` is the offline fallback corpus. Pass an empty array to run strictly
 * fresh-first (see NewsDashboardLive) instead of mutating the shared `seedNews`
 * module array, which would also empty it for every other importer.
 */
export function NewsDashboard({ seeds = seedNews }: { seeds?: NewsItem[] } = {}) {
  ...
  const [news, setNews] = useState<NewsWithReceipt[]>(() => seeds.filter(isDisplayableNews));
  ...
  const loadLiveNews = useCallback(async () => {
    ...
    try {
      ...
      setNews(mergeNews(payload.news, seeds));
      setStatus(payload.errors.length === 0 ? "live" : "partial");
    } catch {
      setNews((current) => (current.length ? current : seeds.filter(isDisplayableNews)));
      setStatus("offline");
    }
  }, [seeds]);
    ...
    return ratedNews
      .filter(
        (item) =>
          quickView === "none" ||
          (quickView === "critical"
            ? item.priority === "Критично"
            : quickView === "strategic"
              ? item.topics.includes("Стратегия") || item.topics.includes("Геополитика")
              : item.focusEntities.length > 0),
      )
      ...
      .filter(
        (item) =>
          !normalizedQuery ||
          `${item.title} ${item.summary} ${item.source} ${item.focusEntities.join(" ")}`
  ...
  const hasActiveFilters =
    quickView !== "none" ||
    priorityFilter !== "Все уровни" ||
    topicFilter !== "Все темы" ||
    marketFilter !== "Все рынки" ||
    query.length > 0;
  function resetFilters() {
    setQuickView("none");
    setPriorityFilter("Все уровни");
    setTopicFilter("Все темы");
    setMarketFilter("Все рынки");
    setQuery("");
  }
  ...
  return (
    <main className="mgc-surface min-h-[calc(100vh-4rem)] text-zinc-950">
      <div className="mx-auto max-w-[1560px] px-4 py-6 sm:px-6 lg:px-8 lg:py-9">
        <section className="mb-7 border border-zinc-300 bg-white shadow-[0_18px_60px_rgba(18,24,35,0.08)]">
          <div className="grid lg:grid-cols-[minmax(0,1fr)_520px]">
            <div className="relative overflow-hidden bg-[#101114] p-6 text-white sm:p-8 lg:p-10">
              <div className="absolute inset-y-0 right-0 w-40 bg-[linear-gradient(135deg,transparent_35%,rgba(38,95,255,.22)_35%,rgba(38,95,255,.22)_52%,transparent_52%)]" />
              <div className="relative">
                <div className="mb-5 flex items-center gap-3 text-xs font-semibold uppercase tracking-[0.18em] text-zinc-400">
                  <span className="h-0.5 w-8 bg-[#285fff]" /> Стратегический мониторинг
                </div>
                <h1 className="max-w-4xl text-4xl font-black uppercase leading-[0.94] tracking-[-0.055em] sm:text-5xl lg:text-6xl">
                  Окно
                  <br />
                  <span className="text-[#5f84ff]">в Китай</span>
                </h1>
                <p className="mt-6 max-w-2xl text-base leading-7 text-zinc-400">
                  Стратегический мониторинг автопрома России и Китая: SHACMAN, GWM, EVOLUTE, VOYAH, Моторинвест, ЭВИА, геополитика,
                  локализация и поставки.
                </p>
              </div>
            </div>
            <div className="grid bg-[#eceff4] sm:grid-cols-3">
              <Metric
                label="Критично"
                value={counts.critical}
                detail="требуют внимания"
                accent="red"
                active={quickView === "critical"}
                onClick={() => showQuickView("critical")}
              />
              <Metric
                label="Стратегия"
                value={counts.strategic}
                detail="геополитика"
                accent="blue"
                active={quickView === "strategic"}
                onClick={() => showQuickView("strategic")}
              />
              <Metric
                label="Бренды / компании"
                value={counts.focus}
                detail="материалов в фокусе"
                accent="orange"
                active={quickView === "focus"}
                onClick={() => showQuickView("focus")}
              />
            </div>
          </div>
        </section>

        <section id="news-feed" className="scroll-mt-24">
          <div className="min-w-0">
            <div className="mb-5 border border-zinc-300 bg-white p-4 shadow-sm">
              <div className="flex items-center gap-2 border-b border-zinc-200 pb-3 text-xs font-bold uppercase tracking-[0.16em] text-zinc-500">
                <SlidersHorizontal className="size-4 text-[#285fff]" /> Фильтры ленты · сначала свежие
                {hasActiveFilters && (
                  <Button
                    type="button"
                    variant="ghost"
                    ...
                    className="ml-auto h-8 rounded-none text-xs normal-case tracking-normal text-[#1f4ed8] hover:bg-blue-50"
                  >
                    Сбросить
                  </Button>
                )}
              ...
              <div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
                ...
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="h-11 rounded-none border-zinc-300 bg-white pl-9 text-zinc-950 placeholder:text-zinc-400"
                  />
                </div>
              </div>
            </div>
            <div className="mb-3 flex items-center justify-between text-sm text-zinc-500">
              <span>{filteredNews.length} материалов после фильтрации</span>
            ...
            {filteredNews.length ? (
              ...
              <div className="border border-dashed border-zinc-300 bg-white px-6 py-20 text-center">
                <Search className="mx-auto size-8 text-zinc-300" />
                <h2 className="mt-4 font-semibold">Ничего не найдено</h2>
                <p className="mt-2 text-sm text-zinc-500">Измените фильтры или покажите всю ленту.</p>
                <Button type="button" onClick={resetFilters} className="mt-5 rounded-none bg-[#111216] text-white hover:bg-[#285fff]">
...
function FilterSelect({ value, values, onChange }: { value: string; values: readonly string[]; onChange: (value: string) => void }) {
  return (
    <Select value={value} onValueChange={onChange}>
      <SelectTrigger className="h-11 w-full cursor-pointer rounded-none border-zinc-300 bg-white text-zinc-950 hover:bg-zinc-50">
        <SelectValue />
      </SelectTrigger>
      <SelectContent>
        {values.map((item) => (
...
function NewsRow({ item, index }: { item: RatedNews; index: number }) {
  return (
    <article className="group grid gap-4 p-5 transition-colors hover:bg-zinc-50 sm:grid-cols-[56px_minmax(0,1fr)_190px] sm:p-6">
      <div>
        <span className="font-mono text-xs font-bold text-zinc-400">{String(index + 1).padStart(2, "0")}</span>
        <div className="mt-3 h-0.5 w-8 bg-[#285fff]" />
      </div>
      <div className="min-w-0">
        <div className="mb-3 flex flex-wrap items-center gap-2">
          ...
          {item.focusEntities.length ? (
            item.focusEntities.map((entity) => (
              <Badge key={entity} variant="outline" className={focusClass(entity)}>
                {entity}
              </Badge>
            ))
          ) : (
          ...
          {item.topics.slice(0, 2).map((topic) => (
            <span key={topic} className="text-xs font-medium text-zinc-500">
              #{topic.toLocaleLowerCase("ru-RU")}
            </span>
          ))}
          {item.translated && (
            <span className="inline-flex items-center gap-1 text-xs text-violet-700">
              <Languages className="size-3.5" /> ZH → RU
            </span>
          )}
        </div>
        <h2 className="max-w-4xl text-lg font-bold leading-7 text-zinc-950 group-hover:text-[#1f4ed8]">{item.title}</h2>
        <p className="mt-2 max-w-4xl text-[15px] leading-6 text-zinc-600">{compactSummary(item.summary, item.title)}</p>
        {item.originalTitle && (
          <p className="mt-3 line-clamp-2 border-l-2 border-violet-300 pl-3 text-sm leading-6 text-zinc-500">{item.originalTitle}</p>
        )}
      </div>
      <div className="flex items-end justify-between gap-4 sm:flex-col sm:items-end">
        <div className="text-left text-xs sm:text-right">
          <div className="flex flex-col items-start gap-1.5 sm:items-end">
            <p className="font-semibold text-zinc-800">{item.source}</p>
            <SourceTrustBadge sourceType={item.sourceType} />
          </div>
          <p
            className="mt-2 inline-flex items-center gap-1 font-medium text-zinc-600"
            title="Время первого получения новости сервисом «Окно в Китай»"
          >
            <Clock3 className="size-3" /> Получено: {item.receivedAt ? formatDateTime(item.receivedAt) : "нет данных"}
          </p>
          <p className="mt-1 text-zinc-400">Опубликовано: {formatDateTime(item.publishedAt)}</p>
          <p className="mt-1 text-zinc-400">{item.market}</p>
        </div>
        <div className="flex flex-wrap justify-end gap-2">
          <FavoriteButton
            item={{
              ...
              metadata: {
                brand: item.brand,
                focusEntities: item.focusEntities,
                market: item.market,
                source: item.source,
                sourceType: item.sourceType,
              },
            }}
            className="rounded-none border-zinc-300 bg-white text-zinc-700 hover:bg-zinc-100"
          />
          <a
            href={item.url}
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex min-h-10 cursor-pointer items-center gap-1.5 px-1 text-sm font-bold text-[#1f4ed8] hover:text-[#111216] focus-visible:ring-2 focus-visible:ring-[#285fff]"
          >
            Первоисточник <ArrowUpRight className="size-4" />
          </a>
        </div>
      </div>
    </article>
...
function Metric({
  ...
  const accents = {
    red: "border-t-red-600 hover:bg-red-50",
    blue: "border-t-[#285fff] hover:bg-blue-50",
    orange: "border-t-orange-500 hover:bg-orange-50",
  };
  return (
    <button
      type="button"
      onClick={onClick}
      aria-pressed={active}
      className={`cursor-pointer border-b border-r border-zinc-300 border-t-4 p-5 text-left transition-colors focus-visible:z-10 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#285fff] sm:p-6 ${accents[accent]} ${active ? "bg-white shadow-inner" : "bg-transparent"}`}
    >
      <p className="text-xs font-bold uppercase tracking-[0.15em] text-zinc-500">{label}</p>
      <div className="mt-3 flex items-end justify-between gap-3">
        <span className="text-4xl font-black leading-none text-zinc-950">{value}</span>
        <span className="max-w-24 text-right text-xs leading-4 text-zinc-500">{detail}</span>
      </div>
      <span className="mt-4 inline-flex text-xs font-bold text-[#1f4ed8]">Показать новости →</span>
    </button>
  );
}
  ...
  if (status === "loading")
    return (
      <span className="inline-flex items-center gap-1.5">
        <RefreshCw className="size-3.5 animate-spin" /> Обновление ленты
      </span>
    );
  if (status === "offline")
    return (
      <span className="inline-flex items-center gap-1.5">
        <ShieldAlert className="size-3.5" /> Резервная лента
      </span>
    );
  return (
    <span className="inline-flex items-center gap-1.5 text-emerald-700">
      <CheckCircle2 className="size-3.5" /> Лента обновлена
    </span>
  );
}
