      ...
      <div className="mx-auto max-w-[1500px] px-4 py-8 sm:px-6 lg:px-8">
        ...
        {report && <>
          ...
          <section className="mt-5 grid gap-5 xl:grid-cols-[1.2fr_0.8fr]">
            ...
            <div className="border border-zinc-300 bg-white p-5"><p className="text-xs font-black uppercase tracking-[0.12em] text-[#285fff]">Использование ключевых контуров</p><div className="mt-5 space-y-4"><Bar label="Decision Cockpit" value={report.usage.decisionViews} /><Bar label="Executive Brief" value={report.usage.executiveViews} /><Bar label="Truck Radar" value={report.usage.truckViews} /><Bar label="Всего telemetry events" value={report.usage.totalEvents} /></div><p className="mt-6 text-xs leading-5 text-zinc-500">Это usage telemetry для оценки пилота, а не рейтинг производительности сотрудников.</p></div>
          </section>
        </>}

        <section className="mt-5 grid gap-5 xl:grid-cols-2">
          <div className="border border-zinc-300 bg-white p-5"><p className="text-xs font-black uppercase tracking-[0.12em] text-[#285fff]">Issue register</p><h2 className="mt-1 text-2xl font-black">S1–S4</h2>{canManageIssues ? <div className="mt-5 space-y-3">{issues.length ? issues.map((issue) => <div key={issue.id} className="border border-zinc-200 p-3"><div className="flex items-center justify-between gap-3"><span className={`text-xs font-black ${issue.severity === "S1" || issue.severity === "S2" ? "text-red-700" : "text-zinc-600"}`}>{issue.severity} · {issue.status}</span><span className="text-xs text-zinc-400">{new Date(issue.updatedAt).toLocaleDateString("ru-RU")}</span></div><p className="mt-2 font-bold">{issue.title}</p><p className="mt-1 text-xs leading-5 text-zinc-500">{issue.description}</p></div>) : <p className="text-sm text-zinc-500">Issues пока не зарегистрированы.</p>}</div> : <p className="mt-4 text-sm text-zinc-500">Детальный issue register доступен администратору. В агрегированных KPI выше видны только количества S1–S4.</p>}</div>
