...
function corporateIdentityError(request: Request, context: ReturnType<typeof createRequestContext>) {
  const principal = resolvePrincipal(request);
  if (principal.mode !== "proxy" || principal.authenticated) return null;
  return jsonWithContext(context, { error: "Для corporate pilot требуется корпоративная аутентификация." }, { status: 401, headers: { "cache-control": "no-store" } });
}

export async function GET(request: Request) {
  const context = createRequestContext(request);
  const identityError = corporateIdentityError(request, context);
